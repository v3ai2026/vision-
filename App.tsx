...
const App: React.FC = () => {
  ...
  const [githubRepoIsPrivate, setGithubRepoIsPrivate] = useState(false);
  ...
  const [githubDeployProgress, setGithubDeployProgress] = useState<string>('');
  ...
  const handleGithubValidate = async () => {
    ...
    try {
      ...
      alert(`GitHub 认证错误：请检查您的 token 是否有效。${errorMessage.includes('Unauthorized') ? '未授权访问' : ''}`);
      setGithubUser(null);
    } finally {
  ...
  const handleGithubDeploy = async () => {
    if (!githubToken || !generationResult || !githubRepoName) return;
    setIsGithubDeploying(true);
    setGithubDeployProgress('');
    setGithubRepoUrl('');
    ...
    try {
      ...
      setGithubDeployProgress('验证 GitHub 用户...');
      const user = await github.getAuthenticatedUser();
      setGithubUser(user);
      
      // Step 2: Create repository
      setGithubDeployProgress('创建 GitHub 仓库...');
      // Sanitize inputs to prevent injection and ensure valid repository descriptions
      const sanitizeInput = (input: string) => input.replace(/[<>'"]/g, '').trim();
      const projectName = generationResult.projectName || 'Generated Project';
      const description = sanitizeInput(
        githubRepoDescription || `Generated by Neural Agent OS - ${projectName}`
      ).substring(0, 350); // GitHub description limit
      const repo = await github.provisionProject(githubRepoName, description, githubRepoIsPrivate);
      const repoUrl = repo.html_url;
      setGithubRepoUrl(repoUrl);
      
      // Step 3: Push files
      setGithubDeployProgress('推送代码文件到仓库...');
      const files = generationResult.files.map(f => ({
        path: f.path,
        content: f.content
      }));
      await github.pushAtomicUpdate(user.login, githubRepoName, files, `feat: initial project setup - ${sanitizeInput(projectName)}`);
      
      // Step 4: Inject CI/CD workflow
      setGithubDeployProgress('注入 GitHub Actions 工作流...');
      await github.injectCIWorkflow(user.login, githubRepoName);
      
      setGithubDeployProgress('部署完成！');
      alert(`成功创建并部署到 GitHub 仓库：${repoUrl}`);
    } catch (e) {
      console.error('GitHub deployment error:', e);
      const errorMessage = e instanceof Error ? e.message : '发生未知错误';
      // Provide user-friendly error messages
      let userMessage = 'GitHub 仓库创建失败。';
      if (errorMessage.includes('Unauthorized')) {
        userMessage += '请检查您的 token 权限。';
      } else if (errorMessage.includes('already exists')) {
        userMessage += '该仓库名称已存在，请使用其他名称。';
      } else {
        userMessage += '请稍后重试。';
      }
      alert(userMessage);
      setGithubDeployProgress('部署失败');
    } finally {
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <div className={`flex-1 overflow-y-auto custom-scrollbar bg-[#020420] ${isMobile ? 'pb-20' : ''}`}>
          ...
          {activeTab === TabType.DEVOPS_DEPLOY && (
            <div className="p-4 md:p-12 animate-modal-fade max-w-7xl mx-auto space-y-8 md:space-y-12">
              <div className="text-center md:text-left space-y-4">
                <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter uppercase leading-none">Deployment Hub</h2>
                <p className="text-slate-500 font-black uppercase tracking-widest text-[10px]">GitHub & Vercel Orchestration Protocol</p>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* GitHub Card */}
                <GlassCard className="space-y-6 h-fit">
                  <div className="flex items-center justify-between">
                    <h3 className="text-xs font-black text-[#00DC82] uppercase tracking-[0.3em]">GitHub Repository</h3>
                    {githubUser && (
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 bg-[#00DC82] rounded-full animate-pulse"></div>
                        <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Authenticated</span>
                      </div>
                    )}
                  </div>

                  {githubUser && (
                    <div className="flex items-center gap-3 p-4 rounded-2xl bg-[#00DC82]/5 border border-[#00DC82]/20">
                      <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-lg">
                        👤
                      </div>
                      <div>
                        <div className="text-sm font-bold text-white">{githubUser.login}</div>
                        <div className="text-[9px] text-slate-500 uppercase tracking-widest">{githubUser.name || 'GitHub User'}</div>
                      </div>
                    </div>
                  )}
                    ...
                    <>
                      <div className="border-t border-white/5 pt-6 space-y-4">
                        <NeuralInput 
                          ...
                          placeholder={(generationResult?.projectName || 'my-project').toLowerCase().replace(/\s+/g, '-')}
                        />
                        ...
                        <NeuralTextArea 
                          ...
                          placeholder="自动生成的项目描述..."
                          className="h-24"
                        />

                        <div className="flex items-center justify-between p-4 rounded-2xl bg-black/40 border border-white/5">
                          <div className="space-y-1">
                            <div className="text-sm font-bold text-white">私有仓库</div>
                            <div className="text-[9px] text-slate-500 uppercase tracking-widest">限制公开访问</div>
                          </div>
                          <NeuralSwitch 
                            checked={githubRepoIsPrivate} 
                            onChange={setGithubRepoIsPrivate} 
                            label="" 
                          />
                        </div>
                        ...
                        <NeuralButton 
                          ...
                          loading={isGithubDeploying} 
                          disabled={!githubRepoName}
                          className="w-full"
                        >
                          创建仓库并部署
                        </NeuralButton>

                        {githubDeployProgress && (
                          <div className="p-4 rounded-2xl bg-black/40 border border-white/5 space-y-2">
                            <div className="flex items-center gap-2">
                              {isGithubDeploying && <NeuralSpinner size="xs" />}
                              <span className="text-xs font-bold text-white">{githubDeployProgress}</span>
                            </div>
                          </div>
                        )}

                        {githubRepoUrl && !isGithubDeploying && (
                          <div className="p-6 rounded-2xl bg-[#00DC82]/10 border border-[#00DC82]/30 space-y-3 animate-modal-slide">
                            <div className="flex items-center gap-2">
                              <span className="text-2xl">🎉</span>
                              <span className="text-sm font-black text-[#00DC82]">部署成功！</span>
                            </div>
                            <a 
                              href={githubRepoUrl} 
                              target="_blank" 
                              rel="noopener noreferrer"
                              className="block text-xs text-white hover:text-[#00DC82] transition-colors break-all underline"
                            >
                              {githubRepoUrl}
                            </a>
                          </div>
                  ...
                  {!generationResult && (
                    <div className="p-6 rounded-2xl bg-black/40 border border-white/5 text-center">
                      <p className="text-xs text-slate-500">首先在 Build 标签页生成项目代码</p>
                    </div>
                  )}
                </GlassCard>
                ...
                <GlassCard className="space-y-6 h-fit">
                  <div className="flex items-center justify-between">
                    <h3 className="text-xs font-black text-[#00DC82] uppercase tracking-[0.3em]">Vercel Deployment</h3>
                    {deployStatus && (
                      <NeuralBadge variant={deployStatus.state === 'READY' ? 'primary' : 'secondary'}>
                        {deployStatus.state}
                      </NeuralBadge>
                    )}
                  </div>
                    ...
                    <>
                      ...
                      {deployStatus && (
                        <div className="p-6 rounded-2xl bg-black/40 border border-white/5 space-y-3">
                          <div className="space-y-1">
                            <div className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Deployment ID</div>
                            <div className="text-xs font-mono text-white">{deployStatus.id}</div>
                          </div>
                          <div className="space-y-1">
                            <div className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Status</div>
                            <div className="text-xs font-bold text-white">{deployStatus.state}</div>
                          </div>
                          {deployStatus.url && (
                            <div className="space-y-1">
                              <div className="text-[8px] font-black text-slate-500 uppercase tracking-widest">URL</div>
                              <a 
                                href={`https://${deployStatus.url}`} 
                                target="_blank" 
                                rel="noopener noreferrer"
                                className="text-xs text-[#00DC82] hover:underline break-all"
                              >
                                {deployStatus.url}
                              </a>
                            </div>
              ...
              <GlassCard className="space-y-6">
                <h3 className="text-xs font-black text-[#00DC82] uppercase tracking-[0.3em]">Setup Instructions</h3>
                
                <div className="space-y-4 text-xs text-slate-300 leading-relaxed">
                  <div className="space-y-2">
                    <h4 className="font-bold text-white">GitHub Personal Access Token</h4>
                    <ol className="list-decimal list-inside space-y-1 pl-2">
                      <li>访问 GitHub Settings → Developer settings → Personal access tokens</li>
                      <li>点击 "Generate new token (classic)"</li>
                      <li>选择以下权限范围：<code className="text-[#00DC82] bg-black/40 px-1 py-0.5 rounded">repo</code>, <code className="text-[#00DC82] bg-black/40 px-1 py-0.5 rounded">user</code>, <code className="text-[#00DC82] bg-black/40 px-1 py-0.5 rounded">workflow</code></li>
                      <li>复制生成的 token 并粘贴到上方输入框</li>
                    </ol>
                  </div>

                  <div className="space-y-2">
                    <h4 className="font-bold text-white">Vercel Token</h4>
                    <ol className="list-decimal list-inside space-y-1 pl-2">
                      <li>登录 Vercel Dashboard</li>
                      <li>进入 Settings → Tokens</li>
                      <li>创建新的 token 并复制</li>
                      <li>粘贴到上方 Vercel Token 输入框</li>
                    </ol>
                  </div>
                </div>
              </GlassCard>
